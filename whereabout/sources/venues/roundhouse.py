...
_CFG = load_venue_config("venue_roundhouse")
_URL = _CFG["url"]
...
def _parse_roundhouse_date(text: str) -> tuple[int, int, int] | None:
    """Parse 'Sun 24 May 26' -> (day, month, year). Returns None on failure."""
    try:
...
class RoundhouseSource(BaseSource):
    source_id = "venue_roundhouse"
    ...
    def _fetch_sync(self, query: Query) -> list[RawEvent]:
        ...
        for card in soup.select("div.event-card"):
            try:
                title_el = card.select_one("h3.event-card__title, h2.event-card__title, h4.event-card__title")
                if not title_el:
                    continue
                ...
                if not date_el:
                    continue
                parsed = _parse_roundhouse_date(date_el.get_text(strip=True))
                if not parsed:
                    continue

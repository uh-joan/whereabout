...
_URL = "https://thejazzcafe.co.uk/events/"
_POSTCODE = "NW1 7NE"
_VENUE = "Jazz Cafe"
...
class JazzCafeSource(BaseSource):
    ...
    def _fetch_sync(self, query: Query) -> list[RawEvent]:
        ...
        try:
            ...
            page.wait_for_selector("article.event, .event-card, li.event", timeout=10000)
            html = page.content()
        except Exception:
            return []
        finally:
        ...
        for el in soup.select("article, li.event, .event-item, .event-card"):
            try:
                ...
                if ticket_url and ticket_url.startswith("/"):
                    ticket_url = "https://thejazzcafe.co.uk" + ticket_url
                try:

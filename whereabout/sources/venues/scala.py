...
_CFG = load_venue_config("venue_scala")
_URL = _CFG["url"]
...
# "Sat 23rd May" — strip ordinal suffix before parsing
_ORDINAL_RE = re.compile(r"(\d+)(?:st|nd|rd|th)", re.I)
_DATE_RE = re.compile(r"(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\d+(?:st|nd|rd|th)?\s+\w+", re.I)


def _parse_scala_date(text: str) -> tuple[int, int] | None:
    m = _DATE_RE.search(text)
    if not m:
        return None
    clean = _ORDINAL_RE.sub(r"\1", m.group(0))
    try:
        dt = datetime.strptime(clean, "%a %d %b")
        return dt.day, dt.month
    except ValueError:
        return None


class ScalaSource(BaseSource):
    source_id = "venue_scala"
    ...
    def _fetch_sync(self, query: Query) -> list[RawEvent]:
        ...
        for card in soup.select(".tb-event-item"):
            try:
                ...
                parsed = _parse_scala_date(full_txt)
                if not parsed:
                    continue
                day, month = parsed
